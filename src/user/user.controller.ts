...
export class UserController {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('admin', 'superadmin')
  async findOne(@Param('id') id: string): Promise<UserDocument> { // Hozircha UserDocument qoldiramiz
    const user = await this.userService.findById(id);
    if (!user) { // Agar UserService dan null qaytsa, bu yerda tekshiramiz
  ...
  async update(@Param('id') id: string, @Body() updateUserDto: UpdateUserDto): Promise<UserDocument> { // Hozircha UserDocument qoldiramiz
    ...
    if (!updatedUser) { // Agar UserService dan null qaytsa, bu yerda tekshiramiz
      throw new NotFoundException(`Foydalanuvchi topilmadi: ID ${id}`);
    }
    return updatedUser;
  }
  ...
  async remove(@Param('id') id: string): Promise<void> {
    // userService.remove allaqachon NotFoundException tashlaydi.
