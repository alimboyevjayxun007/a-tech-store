...
async login(@Body() loginDto: LoginDto, @Res({ passthrough: true }) res: Response) {
  ...
  async logout(@Res({ passthrough: true }) res: Response, @GetUser() user: UserDocument) { // UserDocument ga o'zgartirildi
  ...
  async refreshTokens(@GetUser() user: UserDocument, @Res({ passthrough: true }) res: Response) { // UserDocument ga o'zgartirildi
  ...
  @ApiResponse({ status: 401, description: 'Avtorizatsiya qilinmagan.' })
  getProfile(@GetUser() user: UserDocument) { // UserDocument ga o'zgartirildi
    return user;
  }
