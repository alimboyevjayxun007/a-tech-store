...
import { JwtAuthGuard } from '../auth/guards/auth.guard'; // Yo'l to'g'ri ekanligini tekshiring
...
export class LikeController {
  ...
  async create(@Body() createLikeDto: CreateLikeDto, @Req() req: Request) {
    // MUAMMOni HAL QILISH JOYI: req.user._id yoki req.user.id dan oling
    const userId = (req.user as any)._id || (req.user as any).id;
    console.log(`[LikeController] create method: userId from req.user: ${userId}`); // Debugging uchun log
    if (!userId) {
      throw new Error('Foydalanuvchi IDsi topilmadi. Token to`g`ri konfiguratsiyalanmagan.');
    }
    return this.likeService.create(userId.toString(), createLikeDto.product); // toString() qo'shildi
  }
  ...
  async remove(@Param('productId') productId: string, @Req() req: Request) {
    const userId = (req.user as any)._id || (req.user as any).id;
    console.log(`[LikeController] remove method: userId from req.user: ${userId}`); // Debugging uchun log
    if (!userId) {
      throw new Error('Foydalanuvchi IDsi topilmadi.');
    }
    await this.likeService.remove(userId.toString(), productId); // toString() qo'shildi
  }
  ...
  async getUserLikedProducts(@Req() req: Request) {
    // MUAMMOni HAL QILISH JOYI: req.user._id yoki req.user.id dan oling
    const userId = (req.user as any)._id || (req.user as any).id;
    console.log(`[LikeController] getUserLikedProducts method: userId from req.user: ${userId}`); // Debugging uchun log
    if (!userId) {
      throw new Error('Foydalanuvchi IDsi topilmadi. Avtorizatsiyadan o\'ting.');
    }
    return this.likeService.getUserLikedProducts(userId.toString()); // toString() qo'shildi
  }
  ...
  async checkIfLiked(@Param('productId') productId: string, @Req() req: Request) {
    const userId = (req.user as any)._id || (req.user as any).id;
    console.log(`[LikeController] checkIfLiked method: userId from req.user: ${userId}`); // Debugging uchun log
    if (!userId) {
      throw new Error('Foydalanuvchi IDsi topilmadi.');
    }
    const isLiked = await this.likeService.isProductLikedByUser(userId.toString(), productId); // toString() qo'shildi
    return { isLiked };
  }
}